...
class TrajnetEvaluator:
    ...
    def aggregate(self):
        ...
        for i in range(len(self.scenes_gt)):
            ...
            valid_neighbour = np.array([neighbours_tracks[k][0][1] for k in range(len(neighbours_tracks))])
            ...
            if not self.disable_collision:
                ...
                for j in range(1, len(ground_truth)):
                    if ground_truth[j][0][1] in valid_neighbour:
                        if trajnetplusplustools.metrics.collision(ground_truth[0], ground_truth[j], n_predictions=self.pred_length):
                            continue
                        else:
                ...
                if self.enable_col1:
                    collision_neighbors = np.array([ground_truth[k+1][0][1] for k in range(len(ground_truth)-1) if trajnetplusplustools.metrics.collision(ground_truth[0], ground_truth[k+1], n_predictions=self.pred_length)])
                    
                    for j in range(len(neighbours_tracks)):
                        if neighbours_tracks[j][0][1] in collision_neighbors:
                            continue
                        else:
                            if trajnetplusplustools.metrics.collision(primary_tracks, neighbours_tracks[j], n_predictions=self.pred_length):
                                collision_distance_square = np.ones(self.pred_length)
                                for i in range(self.pred_length):
                                    collision_distance_square[i] = ((primary_tracks[i][2]-neighbours_tracks[j][i][2])**2 + (primary_tracks[i][3]-neighbours_tracks[j][i][3])**2)
