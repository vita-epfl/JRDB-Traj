    ...
    def __call__(self, paths, scene_goal, n_predict=12, modes=1, predict_all=True, obs_length=9, start_length=0, args=None):
        ...
        with torch.no_grad():
            ...
            if args.normalize_scene:
                observed_xy, rotation, center, scene_goal = center_scene(observed_xy, obs_length, goals=scene_goal)
            
            observed_xy = torch.Tensor(observed_xy)  #.to(self.device)
            scene_goal = torch.Tensor(scene_goal) #.to(device)
            batch_split = torch.Tensor(batch_split).long()

            multimodal_outputs = {}
            for num_p in range(modes):
                _, output_scenes = self.model(observed_xy[start_length:obs_length], scene_goal, batch_split, n_predict=n_predict)
