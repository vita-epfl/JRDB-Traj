...
from .lstm import LSTM, LSTMPredictor, drop_distant_far, keep_valid_neigh
...
class Trainer(object):
    ...
    def train(self, scenes, epoch):
        ...
        for scene_i, (filename, scene_id, paths) in enumerate(scenes):
            ...
            if scene[self.obs_length-2,0,-1]==0 or scene[self.obs_length-1,0,-1]==0:
                continue
            scene = keep_valid_neigh(scene)
            scene, _ = drop_distant_far(scene)

            if self.augment:
                scene[:,:,:2] = random_rotation(scene[:,:,:2])
    ...
    def train_batch(self, batch_scene, batch_split):
        ...
        targets_last = batch_scene[self.obs_length-1:self.seq_length-1,:,:].clone()
        targets_last[:,:,-1] = 0
        targets = batch_scene[self.obs_length:self.seq_length] - targets_last
     
        rel_outputs, outputs = self.model(observed, batch_split, prediction_truth)
    ...
    def val_batch(self, batch_scene, batch_split):
        ...
        targets = batch_scene[self.obs_length:self.seq_length]
        targets[:,:,:2] -= batch_scene[self.obs_length-1:self.seq_length-1,:,:2]

        with torch.no_grad():
            ## groundtruth of neighbours provided (Better validation curve to monitor model)
            rel_outputs, _ = self.model(observed, batch_split, prediction_truth)
...
def main(epochs=15):
    ...
    parser.add_argument('--loss', default='L2', choices=('L2'),
