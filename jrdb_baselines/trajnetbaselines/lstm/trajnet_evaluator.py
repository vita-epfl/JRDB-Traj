...
def predict_scene(predictor, model_name, paths, scene_goal, args):
    ...
    predictions = predictor(paths, n_predict=args.pred_length, obs_length=args.obs_length, modes=args.modes, args=args)
    return predictions
...
def get_predictions(args):
    ...
    for model in args.output:
        ...
        if not os.path.exists(args.path + model_name+'/temp'):
            os.makedirs(args.path + model_name+'/temp')
        if not os.path.exists(args.path + model_name+'/jrdb_submission'):
            os.makedirs(args.path + model_name+'/jrdb_submission')
        else:
            print('Predictions corresponding to {} already exist.'.format(model_name))
            print('Delete the existing {} under test_pred/.'.format(model_name))
            continue
        ...
        for dataset in datasets:
            ...
            write_predictions(pred_list, scenes, model_name, dataset_name, dataset_index,args)
            dataset_index+=1
...
def main():
    ...
    parser.add_argument('--output_name', default='Results.png',
                        help='name of outputed evaluation results')
